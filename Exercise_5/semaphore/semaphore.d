...
class Resource(T)
{
    ...
    {
        mtx.wait();
        while (busy || (priority == 0 && numWaiting[1] > 0))
        {
            numWaiting[priority]++; // Increment the number of waiting threads
            mtx.notify();
            sems[priority].wait(); // Wait on the semaphore while holding the mutex
            mtx.wait();
            numWaiting[priority]--; // Decrement the number of waiting threads
        }
        busy = true;
        mtx.notify(); // Release the mutex after acquiring the resource
        return value;
    }
    ...
    void deallocate(T v)
    {
        ...
        if (numWaiting[1] > 0)
        {
            sems[1].notify(); // Signal the semaphore for high priority if there are waiting threads
        }
        else if (numWaiting[0] > 0)
        {
            sems[0].notify(); // Signal the semaphore for low priority if there are waiting threads
        }

        mtx.notify(); // Release the mutex after signaling the semaphore
    }
